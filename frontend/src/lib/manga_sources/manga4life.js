...
function get_data(rawHtml, keyword, endChar) {
   const startIndex = rawHtml.indexOf(keyword);
   if (startIndex !== -1) {
      const remainingString = rawHtml.substring(startIndex + keyword.length).trim();
      const endIndex = remainingString.indexOf(endChar+';');
      if (endIndex !== -1) {
         return JSON.parse(remainingString.substring(0, endIndex + 1).trim());
      }
   }
}
/**
...
export async function search(name) {
   let retrieved;
      let query;
   try {
      const response = await http.fetch(`https://manga4life.com/search/?sort=s&desc=false&name=${name}`, {
        method: 'GET',
        headers: {
      ...
      query = get_data(rawHtml, "vm.Search =", '}').SeriesName;
      console.log(query);
   } catch (error) {
